...
class TestLikes(unittest.TestCase):
    def _make_record_dict(
        ...
        args_dict = {
            ...
            "tweet_user_id": f"tweet_user_id_{index}",
            "tweet_user_name": f"tweet_user_name_{index}",
            "tweet_screen_name": f"tweet_screen_name_{index}",
    ...
    def test_init(self):
        ...
        self.assertEqual(record_dict["tweet_user_id"], instance.tweet_user_id)
        self.assertEqual(record_dict["tweet_user_name"], instance.tweet_user_name)
        self.assertEqual(record_dict["tweet_screen_name"], instance.tweet_screen_name)
    ...
    def test_create(self):
        record_dict = self._make_record_dict()
        instance = Likes.create(record_dict)
        self.assertEqual(record_dict, instance.to_dict())

        with self.assertRaises(ValueError):
            instance = Likes.create("invalid")
    ...
    def test_repr(self):
        ...
        expect = f"<Likes(id='{tweet_id}', screen_name='{screen_name}')>"
        self.assertEqual(expect, actual)

    def test_eq(self):
        record_dict = self._make_record_dict(0)
        instance_1 = Likes.create(record_dict)
        instance_2 = Likes.create(record_dict)
